...
export function CardQuizHome({ props }: { props: { quiz: QuizGet } }) {
  return (
    <Card
      ...
      <Stack>
        <Group>
          <Tooltip label={props.quiz.title} position="top-start" arrowOffset={16}>
            <Title order={2} fw={500} fz={'md'} lineClamp={1}>
              {props.quiz.title}
            </Title>
          </Tooltip>
        </Group>
        ...
        <Group mt={'md'}>
          <Anchor component={Link} href={`/quizzes/${props.quiz.id}`} underline="hover">
            <Group gap={5}>
