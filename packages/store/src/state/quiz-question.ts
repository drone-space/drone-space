...
import type { QuizQuestionGet } from '@repo/types';
...
export const useStoreQuizQuestion = create<QuizQuestionState>((set) => ({
  ...
  updateQuizQuestion: (data) => {
    set((state) => ({
      quizQuestions:
        state.quizQuestions?.map((i) => (i.id === data.id ? { ...data } : i)) ?? undefined,
    }));
  },

  mergeQuizQuestions: (incomingQuizQuestions) => {
    set((state) => {
      if (!incomingQuizQuestions || incomingQuizQuestions.length === 0) return state;
      ...
      const incomingMap = new Map(incomingQuizQuestions.map((n) => [String(n.id), n]));
  ...
  deleteQuizQuestion: (data) => {
    set((state) => ({
      ...
      quizQuestions: state.quizQuestions?.filter((i) => i.id !== data.id) ?? undefined,
    }));
  },
}));
