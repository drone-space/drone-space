...
export default function SpacesHub() {
  ...
  return (
    ...
    >
      ...
      <Divider orientation={!desktop ? undefined : 'vertical'} my={!desktop ? 'xl' : undefined} />

      {data.map((panel, index) => (
        <TabsPanel key={index} value={linkify(panel.title)}>
          <Grid gap={'xl'} pl={{ md: 'md', lg: 'xl' }}>
              ...
              <Stack>
                {panel.list.map((item, index) => (
                  <Group key={index} wrap="nowrap" align="start">
                    <ThemeIcon size={ICON_WRAPPER_SIZE / 1.25} color="sec.3" c={'pri.9'}>
                      <IconArrowRightDashed size={ICON_SIZE / 1.25} stroke={ICON_STROKE_WIDTH} />
                    </ThemeIcon>
...
const data = [
  ...
  },
  {
    ...
    image: 'https://cdn.pixabay.com/photo/2018/03/04/15/33/drone-3198324_1280.jpg',
    list: [
  ...
  },
  {
    ...
    image: 'https://cdn.pixabay.com/photo/2017/09/07/08/57/drone-2724257_1280.jpg',
    list: [
