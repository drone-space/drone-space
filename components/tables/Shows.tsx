...
export default function Shows({ data }: { data: { item: string; duration: string | null }[] }) {
	const rows = data.map(item => (
		<TableTr key={item.item}>
			<TableTd w={{ base: "80%", sm: "70%", md: "80%" }} fz={{ base: "xs", lg: "sm" }}>
				{item.item}
			</TableTd>
			<TableTd w={{ base: "80%", sm: "30%", md: "20%" }} fz={{ base: "xs", lg: "sm" }}>
