...
import { isThisWeek, isToday } from '@repo/utils';
...
export default function Student() {
  ...
  const getAttemptScore = (attemptId: string, quizId: string): number => {
    const attemptAnswers = answers?.filter((an) => an.attemptId === attemptId) || [];
    if (attemptAnswers.length === 0) return 0;

    const correctCount = attemptAnswers.filter((aai) => {
      const answerOption = options?.find((oi) => oi.id === aai.optionId);
      return answerOption?.correct;
    }).length;

    const quizQuestionsQuiz = quizQuestions?.filter((qqqi) => qqqi.quizId === quizId) || [];
  ...
  const quizzesPassed = attemptsComplete?.filter((aci) => {
    const attemptQuiz = quizzes?.find((qi) => qi.id === aci.quizId);
    const thresholdPass = attemptQuiz?.passThreshold ?? 0;
    const currentScore = getAttemptScore(aci.id, aci.quizId);
          ...
          <Card bg={'var(--mantine-color-body)'} withBorder>
            <Stack>
              ...
              {loading ? (
                ...
                <Grid>
                  {sortArray(attemptsComplete || [], (i) => i.createdAt, Order.DESCENDING)?.map(
                    (ai, i) =>
                      i < 3 && (
