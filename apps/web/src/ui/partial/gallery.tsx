...
export default function Gallery({ props }: { props: { list: any[] } }) {
  ...
  const divisor = 16;
  const { items, totalPages, activePage, setActivePage } = usePaginate(props.list, divisor);
  ...
  return (
    <>
      <Grid justify="center" gap={'xs'} my={SECTION_SPACING / 2}>
        {items.map((item) => (
      ...
      <Group justify="center" mt={'xl'}>
        <Pagination size={'sm'} value={activePage} onChange={setActivePage} total={totalPages} />
      </Group>
    </>
  );
}
