...
import type { AttemptGet } from '@repo/types';
...
export const useStoreAttempt = create<AttemptState>((set) => ({
  ...
  updateAttempt: (data) => {
    set((state) => ({
      attempts: state.attempts?.map((i) => (i.id === data.id ? { ...data } : i)) ?? undefined,
    }));
  },
  ...
  mergeAttempts: (incomingAttempts) => {
    set((state) => {
      ...
      const incomingMap = new Map(incomingAttempts.map((n) => [String(n.id), n]));
