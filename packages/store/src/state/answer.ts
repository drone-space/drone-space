...
import type { AnswerGet } from '@repo/types';
...
export const useStoreAnswer = create<AnswerState>((set) => ({
  ...
  updateAnswer: (data) => {
    set((state) => ({
      answers: state.answers?.map((i) => (i.id === data.id ? { ...data } : i)) ?? undefined,
    }));
  },
  ...
  mergeAnswers: (incomingAnswers) => {
    set((state) => {
      ...
      const incomingMap = new Map(incomingAnswers.map((n) => [String(n.id), n]));
