...
const categories = [
	...
	{
		image: image.products.drone.dji.m30t.drone,
		title: "DJI Matrice Series",
		value: "matrice",
	},
	{
		image: image.products.drone.autel.evo2.drone,
		title: "Autel Evo Series",
		value: "evo",
	},
];
...
export default function Enterprise() {
	...
	useEffect(() => {
		...
		} else if (filter == "matrice") {
			setProducts(data.products.filter(product => product.filters.make == "matrice"));
		} else if (filter == "evo") {
			setProducts(data.products.filter(product => product.filters.make == "evo"));
		} else {
			...
			<Layout.Section.Main>
				<Container>
					<Stack gap={"xl"}>
						...
						<Grid align="stretch">
							{products
								.filter(item => item.filters.type.includes("enterprise"))
								.map(product => (
									<GridCol
